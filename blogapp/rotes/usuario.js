...
router.post('/registro', (req, res) => {
    ...
    if(req.body.senha != req.body.senha2){
        erros.push({texto: "As senhas nos dois campos devem ser iguais! Tente novamente."})
    }
    ...
    if(erros.length > 0){
        ...
        Usuario.findOne({email: req.body.email}).then((usuario) => {
            if(usuario){
                req.flash("error_msg", "Esse e-mail já foi cadastrado! Tente novamente.")
                res.redirect('/usuarios/rergistro')
            } else{
